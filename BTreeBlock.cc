...
{
  for (int newPosition = _numberOfKeys; newPosition > position; newPosition--)
  {
...
void BTreeBlock::remove(int position)
{
  _numberOfKeys--;
  for (int currentPosition = position; currentPosition < _numberOfKeys; currentPosition++)
  {
    _key[currentPosition] = _key[currentPosition + 1];
    _value[currentPosition] = _value[currentPosition + 1];
    _child[currentPosition + 1] = _child[currentPosition + 2];
  }
}
